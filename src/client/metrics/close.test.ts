...
describe("close timeline", () => {
  ...
  it("averages damage and deaths by turn number", () => {
    const series = closeByTurn([
      fakeRecording({
        recordingId: "a",
        matchId: "m-a",
        turns: [
          { ...fakeRecording().turns[0]!, turn: 1, damageDealt: 2, creaturesDefeated: 0 },
          { ...fakeRecording().turns[0]!, turn: 2, damageDealt: 6, creaturesDefeated: 1 },
        ],
      }),
      fakeRecording({
        recordingId: "b",
        matchId: "m-b",
        turns: [
          { ...fakeRecording().turns[0]!, turn: 1, damageDealt: 0, creaturesDefeated: 0 },
        ],
      }),
    ]);

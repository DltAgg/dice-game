...
function barGeometry(
  ...
  const barWidth = groupPitch - 10;
  const maxCount = Math.max(1, ...summary.buckets.map((row) => row.total));
  return { plotHeight, groupPitch, barWidth, maxCount };
}
...
function BucketDetail({ bucket }: { readonly bucket: DeckCostBucket }) {
  ...
  const mix = formatTypeMix(bucket.byType);
  return (
    <div className="space-y-3">
      {mix.length > 0 ? <p className="text-xs text-stone-300">{mix}</p> : null}
...
export function DeckCostChart({ deck }: { readonly deck: readonly CardId[] }) {
  ...
  const { plotHeight, groupPitch, barWidth, maxCount } = barGeometry(summary);
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="overflow-x-auto">
            <svg
              ...
              role="img"
              aria-label="Deck cost curve by header cost, stacked by card type"
            >
              <title>Deck cost curve — copies per cost, stacked by type</title>
...
function bucketAriaLabel(row: DeckCostBucket): string {
  const mix = formatTypeMix(row.byType);
  const mixSuffix = mix.length > 0 ? `. ${mix}` : "";
  const weight = formatBucketWeight(row);
  const weightSuffix = weight.length > 0 ? `; ${weight}` : "";
  return `Cost ${row.label}: ${String(row.total)} cards${weightSuffix}${mixSuffix}`;
}

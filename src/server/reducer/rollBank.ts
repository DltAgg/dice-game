...
export function queueRollBank(draft: Draft, symbolIds: readonly SymbolInstanceId[]): void {
  draft.rollBankQueue = [...draft.rollBankQueue, ...symbolIds];
}
...
/** Fire deferred on-absorb triggers for every owner’s symbols banked this roll. */
export function flushRollBankQueue(draft: Draft): void {
  ...
  for (const symbolId of draft.rollBankQueue) {
    ...
    const ownerId = symbol.ownerId;
    queueAbsorbTriggers(
      draft,
      ownerId,
      { kind: "player", id: ownerId },
      symbol.symbol,
      symbol.sourceDieId,
      null,
    );
 ...
 * on-roll resolution stack is clear (all owners in the queue).
 */

...
- **Proving cards (already in catalogue):** Arcane — Insight Rune, Sift, Second Wind. Darkness — Dark Pact (self-mill). Luminar — Glimmer and prevent package (spec `009`), Sidestep, Hunting Armour. Corruption — Great Contamination, Wasting Brand, face-marker suite. Toxin — Dose / Venom / apply-toxin package. Martial — War Charge swap, Command, Predator’s Claws, Insignia of Command. Mechanical — Assembly Line, Die Press, Reforge, Stamp, Coupling, Arcane Echo (re-fire). Wild — **no proving card yet**; first pack-feeding card needs `engine-developer` (no token-transfer effect today). Do not treat Hunter’s Collar / Pack / Instinct as pack feeding — they are absorb/attack/generate, not token-share.
